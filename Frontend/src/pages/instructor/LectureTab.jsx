...
const LectureTab = () => {
    ...
    const fileChangeHandler= async(e)=>{
        ...
        if(file){
            ...
            try{
                const res=await axios.post(`http://localhost:5000/api/media/upload-video`,formData,{
      ...
      const editLectureHandler = async (e) => {
        ...
        try {
            setLoading(true)
            const res = await axios.post(`http://localhost:5000/api/course/${courseId}/lecture/${lectureId}`, data, {
                headers: {
    ...
    const removeLectureHandler=async(e)=>{
            ...
            try{
                ...
                const res= await axios.delete(`http://localhost:5000/api/course/lecture/${lectureId}`,
                    {withCredentials:true}
                )
                if(res.data.success){
   ...
   <Card>
    ...
    <CardContent>
        ...
        <div className='flex items-center space-x-2 my-5'>
            <Switch checked={isFree} onCheckedChange={setIsFree} className='bg-red-500'  />
        ...
        <div className='mt-4'>
            <Button disabled={loading} onClick={editLectureHandler} className='bg-[#006D77]'>
                {
