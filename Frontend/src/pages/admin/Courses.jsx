...
const CreateCourse = () => {
  const [form, setForm] = useState({
    ...
    category: '',
    description: '',
    thumbnail: '', // or courseThumbnail if that's what your backend expects
    coursePrice: '', // optional
  });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.courseTitle || !form.instructor || !form.category) {
    ...
    try {
      ...
      console.error(err);
      setError('Error creating course. Please check the server.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="p-8 md:p-12 bg-gray-100 min-h-screen">
      ...
      <div className="bg-white p-8 rounded-2xl shadow-lg max-w-2xl mx-auto border border-gray-200">
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <button
            type="submit"
            className="w-full bg-indigo-600 text-white px-6 py-3 rounded-lg hover:bg-indigo-700 transition font-bold text-lg shadow-md disabled:opacity-50"
            disabled={loading}
          >
